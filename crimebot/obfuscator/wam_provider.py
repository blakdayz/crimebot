...
class WamProvider(WamProviderBase, WAMBuilder):
    def __init__(self, output_dir: os.PathLike):
        ...
        self.launcher_source = Path(LAUNCHER_SOURCE_PATH) if isinstance(LAUNCHER_SOURCE_PATH, str) else LAUNCHER_SOURCE_PATH
        ...
        try:
            with tarfile.open(output_path, "w:gz") as tar:
                ...
                for root, dirs, files in os.walk(env_path):
                    for file in files:
                        if not file.endswith('.pyc'):  # Exclude '.pyc' files, as they will be recompiled again
                            tar.add(os.path.join(root, file),
                                    arcname=os.path.relpath(os.path.join(root, file), env_path))
    ...
    def analyze_output(output_tarball: Path) -> None:
        try:
            with tarfile.open(output_tarball, "r:gz") as tar:
                ...
                payload_pyc = f"{Path(__file__).parent / 'payloads' / WAMConfig.PAYLOAD_NAME}"
                if not any(member.name == payload_pyc for member in tar.getmembers()):
                    raise Exception(f"Payload pyc file {payload_pyc} not found in output tarball")
    ...
    def arm(self) -> None:
        ...
        env["TEMP_DIR"] = subprocess.check_output("mktemp -d", shell=True, text=True).strip()
