...
async def generate_deepfake(
    ...
    if source_file.content_type not in ["image/jpeg", "video/mp4"]:
        raise HTTPException(
            status_code=400,
            detail="Only JPEG images and MP4 videos are supported for source.",
        )
    if target_file.content_type not in ["image/jpeg", "image/png"]:
        raise HTTPException(
            status_code=400, detail="Only JPEG and PNG images are supported for target."
        )

    try:
        # Save source file to a temporary location
        source_path = os.path.join(
            "/tmp", f"source_{os.getpid()}.{source_file.filename.split('.')[-1]}"
        )
        ...
        target_path = os.path.join(
            "/tmp", f"target_{os.getpid()}.{target_file.filename.split('.')[-1]}"
        )
        ...
        response = requests.post(f"{DEEPFAKE_SERVER_URL}/generate", files=files)
