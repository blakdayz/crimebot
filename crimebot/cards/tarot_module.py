...
class TarotModule:
    ...
    def read_tarot_cards(self, card_names, inverse = False, llm_hook:Callable=None):
        ...
        for card_name in card_names:
            ...
            responses.append(self.find_card_meaning(tarot_data, card_name, card_meaning))
    ...
    def load_tarot_data():
        ...
        try:
            ...
            for card in minor_tarot_data:
                tarot_data_dict[card['name']].append((card['positive'], card['negative']))
    ...
    def find_card_meaning(tarot_data, card_name, card_meaning):
        for meaning in card_meaning.split('\n'):
            if any(f"{card_name}'s" in tarot_data[card_name][i] for i in range(len(tarot_data[card_name]))):
                if 'negative' in meaning and not tarot_data[card_name][0].startswith('Reversed'):
                    return tarot_data[card_name][1].replace('Reversed', '')
                elif 'positive' in meaning and tarot_data[card_name][0].startswith('Reversed'):
                    return tarot_data[card_name][0].replace('Reversed', '').replace('The', '').capitalize() + ' (Inverse)'
                else:
